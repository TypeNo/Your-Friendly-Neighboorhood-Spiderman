...
func _on_body_entered(body):
	if body.name == "PlayerBody":
		...
		$AnimationPlayer2.play("enemy/Death")

		# Optional: remove after animation finishes
		$AnimationPlayer2.animation_finished.connect(_on_death_animation_finished)
		
func _on_death_animation_finished(anim_name):
	if anim_name == "enemy/Death":
		queue_free()  # Now safely remove after the animation
